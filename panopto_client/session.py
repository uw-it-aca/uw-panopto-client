...
class SessionManagement(PanoptoAPI):
    def __init__(self):
        super(SessionManagement, self).__init__({
            'wsdl': 'https://%s/Panopto/PublicAPI/4.6/SessionManagement.svc?wsdl' % (settings.PANOPTO_SERVER)
        })
    ...
    def getFoldersList(self, search_query='', sort_by="Name", sort_increasing="true"):
    ...
    def getAllFoldersByExternalId(self, folder_external_ids, provider_names=[]):
