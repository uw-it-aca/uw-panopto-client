...
class UserManagement(PanoptoAPI):
    def __init__(self):
        super(UserManagement, self).__init__({
            'wsdl': 'https://%s/Panopto/PublicAPI/4.6/UserManagement.svc?wsdl' % (settings.PANOPTO_SERVER)
        })
        self._port = 'BasicHttpBinding_IUserManagement'

    def listUsers(self, search_query='', sort_by="UserKey", sort_increasing="true"):
